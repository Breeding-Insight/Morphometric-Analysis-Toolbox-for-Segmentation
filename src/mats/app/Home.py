...
from mats.dataset_export import pairs_from_manifest
...
def _clear_export_zip_cache():
    ...
    if zip_path:
        try:
            ...
            pass
    dataset_path = st.session_state.pop("dataset_zip_path", None)
    st.session_state.pop("dataset_zip_signature", None)
    st.session_state.pop("dataset_manifest", None)
    if dataset_path:
        try:
...
def render_export_section():
    ...
    render_training_dataset_export(run, methods)


def render_training_dataset_export(run, methods):
    """Package the current run's aligned image and mask pairs for training."""
    from mats.dataset_export import read_group_csv, split_counts, write_dataset_zip

    with st.container(border=True):
        st.markdown("**Training dataset (ZIP)**")
        st.caption(
            "Use target-box images and their masks from this run, including saved "
            "specimen adjustments. Labels are generated by MATS; review them before training."
        )
        method = st.selectbox(
            "Labeling method", methods, format_func=SEGMENTATION_METHOD_LABELS.get,
            key="dataset_method",
        )
        pairs = st.session_state.get("viewer_pairs", {}).get(method, ())
        mask_sources = ["measured"]
        if any(pair.get("raw_mask") for pair in pairs):
            mask_sources.append("raw")
        mask_source = st.selectbox(
            "Masks to use", mask_sources,
            format_func=lambda source: (
                "Measured mask" if source == "measured" else "Raw pre-cleanup mask"
            ),
            key="dataset_mask_source",
        )
        format_labels = {
            "png": "Image + PNG mask",
            "yolo-seg": "YOLO segmentation",
            "yolo-detect": "YOLO detection",
            "coco": "COCO segmentation",
        }
        dataset_format = st.selectbox(
            "Dataset format", tuple(format_labels), format_func=format_labels.get,
            key="dataset_format",
        )
        cols = st.columns(3)
        percentages = tuple(int(cols[index].number_input(
            label, min_value=0, max_value=100, value=default, step=1,
            key=f"dataset_percentage_{split}",
        )) for index, (split, label, default) in enumerate((
            ("train", "Train %", 70), ("val", "Validation %", 20),
            ("test", "Test %", 10),
        )))
        seed = int(st.number_input(
            "Random seed", min_value=0, max_value=2_147_483_647, value=42,
            step=1, key="dataset_seed",
        ))
        group_file = st.file_uploader(
            "Optional group CSV (sample_id,group_id)", type="csv",
            key="dataset_group_csv",
        ...
        try:
            ...
            st.error(error)
        candidates = sum(
            bool(pair.get("target_box") and
                 pair.get("mask" if mask_source == "measured" else "raw_mask") and
                 Path(pair["target_box"]).is_file() and
                 Path(pair["mask" if mask_source == "measured" else "raw_mask"]).is_file())
            for pair in pairs
        )
        if not error:
            estimated = split_counts(candidates, percentages)
            st.caption(
                f"{candidates} candidate pair(s) · expected split: "
                f"{estimated['train']} train / {estimated['val']} validation / "
                f"{estimated['test']} test. Validation may exclude unreadable or mismatched files."
            )
            if candidates and (estimated["train"] == 0 or estimated["val"] == 0):
                st.warning(
        ...
        if dataset_format == "yolo-seg":
            st.caption(
                "YOLO polygons cannot represent holes; any affected specimens "
                "will be listed in the dataset manifest."
            )
        source_paths = tuple(
            path for pair in pairs
            for path in (pair.get("target_box"), pair.get(
                "mask" if mask_source == "measured" else "raw_mask"))
            if path and Path(path).is_file()
        )
        try:
            signature = (
                run.get("run_id"), method, mask_source, dataset_format,
                percentages, seed, group_content,
                tuple((str(path), Path(path).stat().st_size, Path(path).stat().st_mtime_ns)
                      for path in source_paths),
            )
        except OSError:
            signature = None
            st.warning("A pair changed on disk. Refresh Export and try again.")
        if st.session_state.get("dataset_zip_signature") != signature:
            old_path = st.session_state.pop("dataset_zip_path", None)
            st.session_state.pop("dataset_zip_signature", None)
            st.session_state.pop("dataset_manifest", None)
            if old_path:
                Path(old_path).unlink(missing_ok=True)
        if st.button(
            "Prepare training dataset", icon=":material/folder_zip:",
            key="prepare_training_dataset",
            disabled=bool(error) or not candidates or signature is None,
        ):
            dest = None
            try:
                with st.spinner("Building training dataset..."):
                    with tempfile.NamedTemporaryFile(
                        prefix="mats_dataset_", suffix=".zip", delete=False,
                    ) as temp_zip:
                        dest = Path(temp_zip.name)
                ...
                if dest is not None:
                    dest.unlink(missing_ok=True)
                st.error(f"Could not prepare training dataset: {exc}")
            else:
                st.session_state["dataset_zip_path"] = str(dest)
                st.session_state["dataset_zip_signature"] = signature
                st.session_state["dataset_manifest"] = manifest
        dataset_path = st.session_state.get("dataset_zip_path")
        manifest = st.session_state.get("dataset_manifest")
        if dataset_path and manifest and Path(dataset_path).is_file():
            counts = manifest["counts"]
            st.success(
                f"Ready: {counts['train']} train / {counts['val']} validation / "
                f"{counts['test']} test; {len(manifest['excluded'])} excluded."
            )
            if manifest["conversion_notes"]:
                st.warning(f"{len(manifest['conversion_notes'])} conversion note(s) in manifest.json.")
            with open(dataset_path, "rb") as dataset_file:
                st.download_button(
                    "Download training dataset", data=dataset_file,
                    file_name=f"mats_training_{method}_{dataset_format}.zip",
                    mime="application/zip", icon=":material/download:",
                    key="download_training_dataset",
                )
